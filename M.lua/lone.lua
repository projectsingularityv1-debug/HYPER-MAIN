-- ========================================================
-- // Lone Hub — Ultimate Aimbot, 2D/3D ESP & Multi-Layer Speed
-- // Designed for Lone & Universal Games
...
local runId = tick()
env.Lone_RunID = runId

if env.Lone_Cleanup then
    pcall(env.Lone_Cleanup)
end

pcall(function()
    for _, v in ipairs(CoreGui:GetChildren()) do
        if v.Name == "Dummy Kawaii" or v.Name == "LoneHubGUI" then v:Destroy() end
    end
...
local Settings = {
    ...
    toggleKeyName = "E",
    lockMode = "Head", -- "Head", "Torso", "LeftArm", "RightArm", "LeftLeg", "RightLeg", "Random"
    fov = 150,
    smoothing = 0.15,
    ...
    espScanWorkspacePlayers = true,
    
    -- Movement & Anti-Rubberband Speed System
    speedEnabled = false,
    customSpeed = 28,
    speedMethod = "LegitWalkSpeed", -- "LegitWalkSpeed", "LinearVelocity", "BodyVelocity", "MicroCFrame"
    infJump = false,
    noclip = false,
...
-- ========================================================
-- // 5. TARGET RESOLVER (workspace.Players + Players Service)
-- ========================================================
local function getAllTargetEntities()
    local targets = {}
    local seenModels = {}

    -- 1. ตรวจสอบโฟลเดอร์ workspace.Players (ถ้าเปิดใช้งาน)
    if Settings.espScanWorkspacePlayers then
        local wsPlayers = Workspace:FindFirstChild("Players")
        if wsPlayers then
            for _, obj in ipairs(wsPlayers:GetChildren()) do
                if obj:IsA("Model") and obj ~= LocalPlayer.Character then
                    ...
                    if hum and hrp and hum.Health > 0 then
                        ...
                        table.insert(targets, {
                            Model = obj,
                            Name = (plr and plr.Name) or obj.Name,
                            DisplayName = (plr and plr.DisplayName) or obj.Name,
                            Player = plr,
                            Humanoid = hum,
                            RootPart = hrp
                        })
    ...
    -- 2. ดึงจาก Players Service ทั่วไป
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character.Parent and not seenModels[plr.Character] then
...
-- ============================================
-- // 7. HELPER FUNCTIONS & COLOR UTILS
-- ============================================
...
local function isTargetVisible(targetPosition, targetModel)
    ...
    if hitPart and targetModel then
...
local function getTargetHitbox(model)
    ...
    if mode == "Random" then
        local parts = {"Head", "UpperTorso", "HumanoidRootPart", "Torso", "LeftUpperArm", "RightUpperArm", "LeftUpperLeg", "RightUpperLeg"}
        local available = {}
    ...
    if mode == "Head" then return model:FindFirstChild("Head")
    elseif mode == "Torso" then return model:FindFirstChild("UpperTorso") or model:FindFirstChild("Torso") or model:FindFirstChild("HumanoidRootPart")
    elseif mode == "LeftArm" then return model:FindFirstChild("LeftUpperArm") or model:FindFirstChild("Left Arm") or model:FindFirstChild("LeftLowerArm")
    elseif mode == "RightArm" then return model:FindFirstChild("RightUpperArm") or model:FindFirstChild("Right Arm") or model:FindFirstChild("RightLowerArm")
    elseif mode == "LeftLeg" then return model:FindFirstChild("LeftUpperLeg") or model:FindFirstChild("Left Leg") or model:FindFirstChild("LeftLowerLeg")
    elseif mode == "RightLeg" then return model:FindFirstChild("RightUpperLeg") or model:FindFirstChild("Right Leg") or model:FindFirstChild("RightLowerLeg")
    end

    return model:FindFirstChild("Head") or model:FindFirstChild("HumanoidRootPart") or model:FindFirstChild("Torso")
end
...
local function getClosestTarget()
    ...
    for _, target in ipairs(getAllTargetEntities()) do
        if isWhitelisted(target.Name) then continue end
        if Settings.teamCheck and target.Player and target.Player.Team and LocalPlayer.Team and target.Player.Team == LocalPlayer.Team then continue end
        ...
        if distance <= closestDistance then
            if Settings.wallCheck and not isTargetVisible(hrp.Position, target.Model) then continue end
...
local function getAimTargetPosition(target)
    if not target or not target.Model then return nil end

    local targetPart = getTargetHitbox(target.Model)
    if not targetPart then return nil end

    if Settings.wallCheck and not isTargetVisible(targetPart.Position, target.Model) then
        return nil
    end
    ...
    return targetPart.Position + (velocity * travelTime * Settings.predictionFactor), targetPart
end
...
local function updateAimbot()
    ...
    if currentTarget and currentTarget.Model and currentTarget.Model.Parent then
        ...
        local hrp = currentTarget.RootPart
        if hum and hrp and hum.Health > 0 then
            local _, onScreen = Camera:WorldToViewportPoint(hrp.Position)
            if onScreen then
                keepTarget = (not Settings.wallCheck) or isTargetVisible(hrp.Position, currentTarget.Model)
            end
        end
    end
    ...
    if currentTarget then
        local targetPosition = getAimTargetPosition(currentTarget)
        ...
        oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, prop)
            if not checkcaller() and Settings.silentAim and tostring(prop) == "Hit" and tostring(self) == "Mouse" then
                ...
                if target and target.Model then
                    local part = getTargetHitbox(target.Model)
                    if part then
                        return part.CFrame
                    end
                end
...
-- ========================================================
-- // 10. ADVANCED ESP SYSTEM (2D Box, 3D Box, Info, Tracers)
-- ========================================================
local espFolder = Instance.new("Folder")
espFolder.Name = "Lone_ESP_Folder"
...
local function createESPInstance(target)
    local model = target.Model
    if not model or espObjects[model] then return end

    local data = {}

    -- Billboard for Names, Distance, Health
    local billboard = Instance.new("BillboardGui")
    billboard.Name = model.Name .. "_Tag"
    ...
    -- 3D Wireframe SelectionBox
    local sBox = Instance.new("SelectionBox")
    sBox.Name = model.Name .. "_3DBox"
    sBox.LineThickness = 0.04
    sBox.Color3 = Settings.espColor
    sBox.SurfaceTransparency = 0.85
    sBox.Visible = false
    sBox.Parent = espFolder

    -- Drawing Tracers & 2D Drawing Box (if supported)
    local drawingBox = nil
    local tracerLine = nil
    if hasDrawing then
        pcall(function()
            drawingBox = Drawing.new("Square")
            drawingBox.Thickness = 1.5
            drawingBox.Filled = false
            drawingBox.Transparency = 1
    ...
    data.NameLbl = nameLabel
    data.DistLbl = distLabel
    data.HealthLbl = healthLabel
    data.Highlight = hl
    data.SelectionBox = sBox
    data.DrawingBox = drawingBox
    data.TracerLine = tracerLine
    data.Target = target

    espObjects[model] = data
end

local function removeESPInstance(model)
...
local function updateESP()
    if not Settings.espEnabled then
        for _, data in pairs(espObjects) do
            ...
            data.SelectionBox.Visible = false
    ...
    local currentEntities = getAllTargetEntities()
    local validModels = {}

    for _, target in ipairs(currentEntities) do
        local model = target.Model
        validModels[model] = true
        if not espObjects[model] then
            createESPInstance(target)
        end
        ...
        if data then
            ...
            local wl = isWhitelisted(target.Name)
            local espColor
            if wl then
            ...
            elseif Settings.useRGBColors and Settings.espRainbow then
                ...
                espColor = getTeamColor(target)
            end

            -- Billboard Update
            data.Billboard.Adornee = head
            data.Billboard.Enabled = (Settings.espNames or Settings.espDistance or Settings.espHealth)
            
            data.NameLbl.Visible = Settings.espNames
            if Settings.espNames then
                data.NameLbl.TextColor3 = espColor
                data.NameLbl.Text = wl and (target.Name .. " [✓ WL]") or target.Name
            end

            data.DistLbl.Visible = Settings.espDistance
            if Settings.espDistance and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                data.DistLbl.TextColor3 = espColor
            end

            data.HealthLbl.Visible = Settings.espHealth
            if Settings.espHealth then
                local hp = math.floor((hum.Health / math.max(1, hum.MaxHealth)) * 100)
                data.HealthLbl.Text = hp .. "% HP"
                data.HealthLbl.TextColor3 = Color3.fromRGB(math.clamp(255 - hp * 2.55, 0, 255), math.clamp(hp * 2.55, 0, 255), 0)
            end
            ...
            data.Highlight.Enabled = Settings.espHighlight

            -- 3D Wireframe Box (SelectionBox)
            data.SelectionBox.Adornee = model
            data.SelectionBox.Color3 = espColor
            data.SelectionBox.Visible = Settings.esp3DBox

            -- Drawing 2D Box & Tracers
            local screenPos, onScreen = Camera:WorldToViewportPoint(hrp.Position)
            if onScreen then
                if data.DrawingBox and Settings.esp2DBox then
                    local headPos = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, 0.5, 0))
                    local legPos = Camera:WorldToViewportPoint(hrp.Position - Vector3.new(0, 2.8, 0))
                    local height = math.abs(headPos.Y - legPos.Y)
                    local width = height * 0.65

                    data.DrawingBox.Size = Vector2.new(width, height)
                    data.DrawingBox.Position = Vector2.new(screenPos.X - width / 2, headPos.Y)
                    data.DrawingBox.Color = espColor
                    data.DrawingBox.Visible = true
                elseif data.DrawingBox then
                    data.DrawingBox.Visible = false
                end

                if data.TracerLine and Settings.espTracers then
                    ...
                    data.TracerLine.Color = espColor
    ...
    -- Clean up dead or removed models
    for model, _ in pairs(espObjects) do
        if not validModels[model] or not model.Parent then
            removeESPInstance(model)
        end
    end
end

-- ========================================================
-- // 11. ANTI-RUBBERBAND SPEED ENGINE (BYPASS SERVER ROLLBACK)
-- ========================================================

-- [11.1] ป้องกันเกมรีเซ็ตค่า WalkSpeed (Metamethod Hook)
pcall(function()
    if hookmetamethod and checkcaller and newcclosure then
        ...
        oldNewIndex = hookmetamethod(game, "__newindex", newcclosure(function(self, prop, val)
            if not checkcaller() and Settings.speedEnabled and tostring(prop) == "WalkSpeed" and self:IsA("Humanoid") then
                local char = LocalPlayer.Character
                if char and self:IsDescendantOf(char) then
                    return oldNewIndex(self, prop, Settings.customSpeed)
                end
            end
...
-- [11.2] Advanced Physics & Anti-Lagback Mover
local bodyVelocityMover = nil

local function cleanupMover()
    if bodyVelocityMover then
        pcall(function() bodyVelocityMover:Destroy() end)
        bodyVelocityMover = nil
    end
end

-- RenderStepped & Stepped Multi-Engine Speed Handler
local speedRenderConn = RunService.RenderStepped:Connect(function(dt)
    if env.Lone_RunID ~= runId then return end
    local char = LocalPlayer.Character
    if not char then cleanupMover(); return end

    local hum = char:FindFirstChildOfClass("Humanoid")
    local hrp = char:FindFirstChild("HumanoidRootPart")

    if Settings.speedEnabled and hum and hrp and hum.Health > 0 then
        -- 1. ปรับค่า WalkSpeed ของ Humanoid
        hum.WalkSpeed = Settings.customSpeed

        local moveDir = hum.MoveDirection
        local isMoving = (moveDir.Magnitude > 0)

        if Settings.speedMethod == "LegitWalkSpeed" then
            -- โหมดเนียนสุด: พึ่งพา Humanoid.WalkSpeed ล้วนๆ ไม่มีการแทรก CFrame ทำให้ไม่โดนเซิร์ฟเวอร์ดึงกลับ (Anti-Rubberband 100%)
            cleanupMover()

        elseif Settings.speedMethod == "LinearVelocity" then
            -- โหมดแรงส่งฟิสิกส์: ส่งแรงทางระนาบ X/Z ตาม AssemblyLinearVelocity ไม่ทำให้ CFrame ข้ามกำแพงป้องกันการ Re-position
            cleanupMover()
            if isMoving then
                local curY = (hrp.AssemblyLinearVelocity and hrp.AssemblyLinearVelocity.Y) or hrp.Velocity.Y
                local targetVel = Vector3.new(moveDir.X * Settings.customSpeed, curY, moveDir.Z * Settings.customSpeed)
        ...
        elseif Settings.speedMethod == "BodyVelocity" then
            -- โหมดสร้าง Physics Mover เสถียรสูง
            if isMoving then
                if not bodyVelocityMover or bodyVelocityMover.Parent ~= hrp then
                    cleanupMover()
                    bodyVelocityMover = Instance.new("BodyVelocity")
                    bodyVelocityMover.Name = "Lone_SpeedMover"
                    bodyVelocityMover.MaxForce = Vector3.new(1e5, 0, 1e5)
                    bodyVelocityMover.Parent = hrp
                end
                bodyVelocityMover.Velocity = Vector3.new(moveDir.X * Settings.customSpeed, 0, moveDir.Z * Settings.customSpeed)
            else
                if bodyVelocityMover then
                    bodyVelocityMover.Velocity = Vector3.zero
                end
            end

        elseif Settings.speedMethod == "MicroCFrame" then
            -- โหมด CFrame แบบเสี้ยวเล็ก (Smooth Interpolation) ป้องกันการกระตุกหลุด Map
            cleanupMover()
            if isMoving then
                local extraSpeed = math.max(0, Settings.customSpeed - 16)
                local stepDistance = moveDir * (extraSpeed * math.clamp(dt, 0, 0.033) * 0.85)
                
                -- ตรวจสอบก่อนว่าข้างหน้าไม่มีกำแพงขวาง ป้องกันเซิร์ฟเวอร์สั่ง Rollback จากการติดกำแพง
                ...
                local rayResult = Workspace:Raycast(hrp.Position, stepDistance.Unit * 2, rayParams)
                
                if not rayResult then
                    hrp.CFrame = hrp.CFrame + stepDistance
                end
            end
        end
    else
...
local steppedConn = RunService.Stepped:Connect(function()
    ...
    if Settings.speedEnabled then
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum and hum.WalkSpeed ~= Settings.customSpeed then
            hum.WalkSpeed = Settings.customSpeed
        end
    end
end)
...
-- ============================================
-- // 12. STANDALONE UI LIBRARY (Dummy Kawaii Engine)
-- ============================================
...
local themes = {
	index = {'Dark', 'Amethyst', 'Rose', 'Ocean', 'Neon', 'Gold'},
Rose = {
...
ScreenGui.Name = (game:GetService("HttpService"):GenerateGUID(false):gsub("-", ""):sub(1, 16))
ScreenGui.ResetOnSpawn = false
ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

-- [STEALTH UI BYPASS v2]: ป้องกัน Anti-Cheat (BAC / Adonis) ตรวจจับ GUI 100%
pcall(function()
    -- 1. ตัดการเชื่อมต่อของ Listener ที่ดักฟัง ChildAdded ใน CoreGui & PlayerGui
    if getconnections then
        for _, conn in pairs(getconnections(game:GetService("CoreGui").ChildAdded)) do
    ...
    -- 2. เลือก Container ที่ปลอดภัยที่สุดตามลำดับ
    if gethui then
...
function Library:Window(p)
	...
	if ProfileData and type(ProfileData) == "table" then
		...
		Profile_Avatar.Image = "rbxassetid://10901594247" -- Default user icon
		...
		if avatarUrl and avatarUrl ~= "" then
			task.spawn(function()
				...
				-- วิธีที่ 2: ถ้าเป็น URL ภายนอก ลองใช้ getcustomasset (executors ที่รองรับ)
				if avatarUrl:match("^https?://") then
					pcall(function()
						if getcustomasset and isfolder and writefile then
							if not isfolder("SingularityAssets") then
								makefolder("SingularityAssets")
							end
...
-- ============================================
-- // 13. INITIALIZE LONE HUB WINDOW & TABS
-- ============================================
...
local SpeedTab = Window:Tab({ Title = "Speed & Movement", Icon = "zap" })
...
-- // 1. AIMBOT TAB
-- ============================================
AimbotTab:Section({ Title = "Aimbot Core Settings" })

AimbotTab:Toggle({
    Title = "Camera Aimbot",
    Desc = "ล็อกเป้าหมายผ่านการหมุนมุมกล้องอัตโนมัติ",
    Image = "crosshair",
...
AimbotTab:Toggle({
    Title = "Silent Aim",
    Desc = "กระสุนเข้าเป้าอัตโนมัติโดยไม่ต้องหันกล้อง (Metamethod Hook)",
    Image = "target",
...
AimbotTab:Keybind({
    ...
    Desc = "คลิกปุ่มด้านขวาแล้วกดคีย์บอร์ดที่ต้องการตั้งค่า",
    Image = "key",
    ...
    Callback = function(key, val)
        if typeof(key) == "EnumItem" then
            ...
            Settings.toggleKeyName = tostring(key):gsub("Enum.KeyCode.", "")
        end
        Settings.Toggle = val
    end
})
...
AimbotTab:Dropdown({
    Title = "Lock Hitbox Target",
    Desc = "ตำแหน่งร่างกายที่จะเล็งยิง",
    Image = "disc",
    List = { "Head", "Torso", "LeftArm", "RightArm", "LeftLeg", "RightLeg", "Random" },
    Value = Settings.lockMode,
    Callback = function(choice) Settings.lockMode = choice end
})

AimbotTab:Section({ Title = "Target Filtering & Checks" })

AimbotTab:Toggle({
    Title = "Wall Check",
    Desc = "ล็อกเฉพาะเป้าหมายที่มองเห็น (ไม่ติดกำแพงบัง)",
    Image = "eye",
...
AimbotTab:Slider({
    Title = "FOV Radius",
    Desc = "ขนาดวงกลมระยะเล็งล็อกเป้า",
    Image = "maximize-2",
    Min = 30,
    Max = 500,
    Value = Settings.fov,
...
AimbotTab:Slider({
    ...
    Desc = "ความนุ่มนวลในการหันมุมกล้องเข้าหาเป้าหมาย",
    Image = "sliders",
    Min = 1,
    Max = 100,
    Value = math.floor(Settings.smoothing * 100),
    Rounding = 0,
    Callback = function(val)
...
AimbotTab:Slider({
    ...
    Desc = "ระบบคำนวณการเคลื่อนที่ล่วงหน้าของเป้าหมาย",
    Image = "activity",
    Min = 0,
    Max = 50,
    Value = math.floor(Settings.predictionFactor * 100),
    Rounding = 0,
    Callback = function(val)
...
AimbotTab:Section({ Title = "FOV Ring Visuals" })

AimbotTab:Toggle({
    Title = "Show FOV Ring",
    Desc = "แสดงวงกลม FOV ระยะล็อกเป้าบนหน้าจอ",
    Image = "circle",
...
AimbotTab:Toggle({
    Title = "Filled FOV Circle",
    Desc = "ถมสีโปร่งแสงด้านในวงกลม FOV",
    Image = "circle-dot",
    Value = Settings.fovFilled,
    Callback = function(v)
...
AimbotTab:ColorPicker({
    ...
    Desc = "เลือกสีวงกลม FOV แบบกำหนดเอง",
    Image = "palette",
    ...
    Callback = function(r, g, b)
        ...
        elseif type(r) == "number" and type(g) == "number" and type(b) == "number" then
            ...
            c = Color3.fromRGB(0, 255, 170)
        end
        Settings.fovColor = c
        if FOVring then FOVring.Color = c end
    end
})
...
-- // 2. ESP & VISUALS TAB
-- ============================================
ESPTab:Section({ Title = "Master ESP & Data Source" })

ESPTab:Toggle({
    Title = "Enable Master ESP",
    Desc = "เปิด/ปิดระบบ ESP แสดงผลตำแหน่งทั้งหมด",
    Image = "eye",
...
ESPTab:Toggle({
    Title = "Scan workspace.Players",
    Desc = "ดึงข้อมูลโมเดลผู้เล่นจาก workspace.Players และ Players Service",
    Image = "search",
    Value = Settings.espScanWorkspacePlayers,
    Callback = function(v) Settings.espScanWorkspacePlayers = v end
})

ESPTab:Section({ Title = "2D & 3D Box ESP" })

ESPTab:Toggle({
    Title = "2D Box ESP (Screen Box)",
    Desc = "แสดงกรอบสี่เหลี่ยม 2D ล้อมรอบตัวละครบนหน้าจอ",
    Image = "square",
    Value = Settings.esp2DBox,
    Callback = function(v) Settings.esp2DBox = v end
})

ESPTab:Toggle({
    Title = "3D Wireframe Box (World Box)",
    Desc = "แสดงกล่อง 3 มิติครอบรอบตัวโมเดลผู้เล่นในโลก 3D",
    Image = "box",
    Value = Settings.esp3DBox,
    Callback = function(v) Settings.esp3DBox = v end
})

ESPTab:Toggle({
    Title = "Highlight Chams",
    Desc = "เรืองแสงทะลุกำแพงรอบตัวละครโมเดล",
    Image = "sun",
    Value = Settings.espHighlight,
    Callback = function(v) Settings.espHighlight = v end
})

ESPTab:Section({ Title = "ESP Information & Tracers" })

ESPTab:Toggle({
    Title = "Player Names",
    Desc = "แสดงชื่อผู้เล่นบนหัวตัวละคร",
    Image = "type",
...
ESPTab:Toggle({
    ...
    Desc = "แสดงระยะห่างเป็นหน่วยเมตร [m]",
    Image = "navigation",
...
ESPTab:Toggle({
    Title = "Health Bar & Percent",
    Desc = "แสดงระดับเลือดและเปอร์เซ็นต์ HP",
    Image = "heart",
...
ESPTab:Toggle({
    ...
    Desc = "ลากเส้นนำสายตาจากกลางล่างจอไปยังเป้าหมาย",
    Image = "git-commit",
    Value = Settings.espTracers,
    Callback = function(v) Settings.espTracers = v end
})

ESPTab:Section({ Title = "Colors & Dynamic Effects" })

ESPTab:Toggle({
    Title = "Team Color Matching",
    Desc = "เปลี่ยนสีตามทีมผู้เล่นอัตโนมัติ (น้ำเงิน/แดง)",
    Image = "users",
    Value = Settings.espTeamColor,
    Callback = function(v) Settings.espTeamColor = v end
})

ESPTab:Toggle({
    Title = "RGB Rainbow Cycle",
    Desc = "เอฟเฟกต์สีรุ้งหมุนวน RGB แบบเรียลไทม์",
    Image = "refresh-cw",
...
ESPTab:Slider({
    ...
    Desc = "ความเร็วในการเปลี่ยนสีรุ้ง",
    Image = "gauge",
    Min = 1,
    Max = 20,
...
ESPTab:ColorPicker({
    Title = "Custom ESP Color",
    Desc = "เลือกสี ESP แบบกำหนดเอง (เมื่อปิดสีรุ้ง/สีทีม)",
    Image = "palette",
    Value = Settings.espColor,
    Callback = function(r, g, b)
        local c
        if typeof(r) == "Color3" then
            c = r
...
-- ============================================
-- // 3. SPEED & MOVEMENT TAB (Anti-Rubberband Engine)
-- ============================================
SpeedTab:Section({ Title = "Anti-Rubberband Speed Engine" })

SpeedTab:Toggle({
    Title = "Enable Speed Hack",
    Desc = "เปิดใช้งานระบบเร่งความเร็วการเคลื่อนที่แบบไม่วาปกลับ",
    Image = "zap",
...
SpeedTab:Slider({
    ...
    Desc = "ความเร็วที่ต้องการ (ค่าแนะนำ 24 - 40 เพื่อความลื่นไหล)",
    Image = "gauge",
    Min = 16,
    Max = 120,
    Value = Settings.customSpeed,
    Rounding = 0,
    Callback = function(val) Settings.customSpeed = val end
})

SpeedTab:Dropdown({
    Title = "Speed Engine Method",
    Desc = "LegitWalkSpeed (ไม่วาปกลับ 100%), LinearVelocity (ส่งแรงฟิสิกส์), BodyVelocity (Mover), MicroCFrame",
    Image = "cpu",
    List = { "LegitWalkSpeed", "LinearVelocity", "BodyVelocity", "MicroCFrame" },
    Value = Settings.speedMethod,
    Callback = function(choice) Settings.speedMethod = choice end
})

SpeedTab:Section({ Title = "Movement Utilities & Godmode" })

SpeedTab:Toggle({
    Title = "Infinite Jump",
    Desc = "กระโดดได้ไม่จำกัดกลางอากาศ (กด Spacebar รัวๆ)",
    Image = "chevrons-up",
...
SpeedTab:Toggle({
    ...
    Desc = "เดินทะลุกำแพงและสิ่งกีดขวางทั้งหมดในเกม",
    Image = "ghost",
...
SpeedTab:Keybind({
    ...
    Desc = "ปุ่มคีย์ลัดสำหรับเปิด/ปิด Godmode",
    Image = "key",
    Key = Settings.godmodeKey,
    Value = Settings.godmode,
    Callback = function(key, val)
        if typeof(key) == "EnumItem" then
            Settings.godmodeKey = key
        end
        toggleGodmode()
    end
})
...
-- // 4. WHITELIST TAB
-- ============================================
WhitelistTab:Section({ Title = "Quick Add / Remove Whitelist" })

WhitelistTab:Textbox({
    Title = "Add Username to Whitelist",
    Desc = "พิมพ์ชื่อผู้เล่นแล้วกด Enter เพื่อเพิ่มเข้า Whitelist",
    Placeholder = "พิมพ์ชื่อผู้เล่นที่นี่...",
    Image = "user-check",
    ClearText = true,
    Callback = function(text)
        if text and text ~= "" then
            if addToWhitelist(text) then
                Window:Notify({ Title = "Whitelist", Desc = "เพิ่ม " .. text .. " แล้ว [✓]", Time = 3 })
            else
                Window:Notify({ Title = "Whitelist", Desc = text .. " อยู่ในรายการแล้ว", Time = 3 })
            end
        end
    end
})
...
WhitelistTab:Dropdown({
    Title = "Select Server Player",
    Desc = "เลือกรายชื่อผู้เล่นจากในเซิร์ฟเวอร์",
    Image = "users",
...
WhitelistTab:Button({
    ...
    Desc = "เพิ่มผู้เล่นที่เลือกใน Dropdown เข้า Whitelist",
    Image = "user-plus",
    Callback = function()
...
WhitelistTab:Button({
    ...
    Desc = "ลบผู้เล่นที่เลือกออกจาก Whitelist",
    Image = "user-minus",
    Callback = function()
        if selectedPlayerName == "(ไม่มีผู้เล่นอื่น)" then return end
...
WhitelistTab:Section({ Title = "Manage Whitelist Data" })

WhitelistTab:Button({
    Title = "View All Whitelist Members",
    Desc = "แสดงรายชื่อที่อยู่ใน Whitelist ทั้งหมดบนแจ้งเตือน",
    Image = "list",
    Callback = function()
        if #Whitelist == 0 then
...
WhitelistTab:Button({
    Title = "Clear All Whitelist",
    Desc = "ล้างข้อมูล Whitelist ทั้งหมดออกทันที",
    Image = "trash-2",
    Callback = function()
        Whitelist = {}
...
-- ============================================
SecurityTab:Section({ Title = "Bypass & Protection Status" })

SecurityTab:Button({
    Title = "Adonis Metatable Check: " .. (AC_Status.AdonisPatched and "[Bypassed]" or "[Active/Safe]"),
...
SecurityTab:Button({
    Title = "LogService Hook: " .. tostring(AC_Status.LogServiceBlocked) .. " Hook(s) Active",
...
SecurityTab:Button({
    Title = "RemoteFunctions Protection: " .. tostring(AC_Status.RemoteFunctionsHooked) .. " Hooked",
...
SecurityTab:Button({
    Title = "Timeout Safeguard: " .. (AC_Status.TimeoutProtected and "[Protected]" or "[Ready]"),
    Desc = "ป้องกันเกมยิง Infinite Loop เพื่อแกล้งให้ตัวรันค้าง",
    Image = "clock",
    Callback = function() end
})

SecurityTab:Section({ Title = "Emergency Action" })

SecurityTab:Button({
    Title = "Panic Button (Emergency Killswitch)",
...
-- // 6. USER INFO TAB
-- ============================================
UserTab:Section({ Title = "Project Information" })
UserTab:Label({ Title = "Project Name:", Desc = "Lone Hub — Elite Multi-Layer Suite", Image = "folder" })
UserTab:Label({ Title = "Version:", Desc = "v2.1 (Anti-Rubberband Engine)", Image = "tag" })
UserTab:Label({ Title = "Creator:", Desc = "K2NTA ST | Project Singularity", Image = "user" })

UserTab:Section({ Title = "Keybindings & Shortcuts" })
UserTab:Label({ Title = "Right Shift:", Desc = "เปิด / ปิดหน้าต่างเมนู UI (หรือกดปุ่ม Close)", Image = "keyboard" })
UserTab:Label({ Title = "Delete Key:", Desc = "Panic Button (ปิดทุกฟีเจอร์ฉุกเฉิน)", Image = "alert-triangle" })
UserTab:Label({ Title = "E Key:", Desc = "Aimbot Activation Key (ปรับเปลี่ยนได้ในแท็บ Aimbot)", Image = "crosshair" })
UserTab:Label({ Title = "G Key:", Desc = "Godmode Quick Toggle (ปรับเปลี่ยนได้ในแท็บ Speed)", Image = "heart" })


-- ============================================
-- // 14. EVENT LISTENERS & MAIN RENDER LOOP
-- ============================================
...
Window:Notify({
    Title = "Lone Hub v2.1 Loaded",
    Desc = "Anti-Rubberband Engine Active | Right Shift = UI",
    Time = 6
})

print("✅ Lone Hub v2.1 Loaded Successfully (Anti-Rubberband Engine)!")
